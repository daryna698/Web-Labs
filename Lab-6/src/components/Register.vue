...
// 4. Реєстрація та лог в консоль
const handleRegister = () => {
  if (validate()) {
    // Вивід у консоль браузера (F12 -> Console)
    console.group('Register Data Submitted')
    console.log('Username:', form.value.username)
    console.log('Password:', form.value.password)
    console.log('Email:', form.value.email)
    console.groupEnd()

    // Формуємо текст для alert, щоб ви відразу побачили дані на екрані
    const alertMessage = `Реєстрація успішна!\n\nВідправлені дані:\nUsername: ${form.value.username}\nEmail: ${form.value.email}\nPassword: ${form.value.password}`
...
<template>
  <div class="flex flex-col min-h-screen bg-[#FF5A5F] px-8 py-6 font-sans">
    ...
    <div class="flex flex-row gap-4 mb-8">
      <button @click="goToLogin" class="flex-1 py-3 rounded-lg font-bold text-sm tracking-wider shadow-sm bg-white/20 text-white hover:bg-white/30 transition-all">
        LOGIN
      </button>
    ...
    <div class="space-y-4 w-full">
      ...
      <div>
        <input
          ...
          placeholder="email"
          class="w-full p-4 rounded-xl bg-[#FFE5E5] text-[#4A4A4A] placeholder-gray-500 font-bold text-center shadow-inner focus:outline-none focus:ring-4 focus:ring-white/40"
          :class="{ 'ring-4 ring-red-800 bg-red-100': errors.email }"
        />
        <p v-if="errors.email" class="text-white text-xs font-bold mt-1 text-center bg-red-800/20 rounded py-1">
          {{ errors.email }}
        </p>
      </div>
    </div>
    ...
    <div class="mt-auto mb-4 text-center pt-8">
      <p class="text-xs text-white/80 font-bold uppercase tracking-wide">
        ...
        <span @click="goToLogin" class="text-white font-black cursor-pointer hover:underline ml-1">Login</span>
      </p>
    </div>
